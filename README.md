...
**새로운 레터헤드 적용**:
- ✅ **공식 레터헤드**: EXIT COMPANY 레터헤드 이미지 사용
- ✅ **수신자 정보 상단 표시**: (사서함주소)-(수용번호) (회원성명)님 귀하
- ✅ **좌측 정렬**: 레터헤드 바로 아래 수신자 정보 표시
- ✅ **수령일 삭제**: 수령일 필드 제거
- ✅ **A4 최적화**: 210mm × 297mm 레이아웃

**템플릿 구조**:
```
┌─────────────────────────────┐
│ EXIT COMPANY 레터헤드       │ ← 이미지
├─────────────────────────────┤
│                             │
│ 남인천 사서함 343-1111      │ ← 좌측 정렬
│ 김테스트님 귀하              │
│                             │
│ [공지사항 (선택)]            │
│                             │
...
**수신자 정보 형식**:
- `po_box_address`: 사서함 주소 (예: 남인천 사서함 343)
- `recipient_number`: 수용번호 (예: 1111)
- `recipient_name`: 회원 성명 (예: 김테스트)
- **출력**: `남인천 사서함 343-1111 김테스트 님 귀하`

**스타일 개선**:
- 레터헤드: 전체 너비, 자동 높이
- 수신자: 14-15pt 폰트, 좌측 정렬, 상단 30px 여백
- 내용: 12pt 폰트, 2.0 줄 간격
- 하단 여백: 40px, 우측 정렬
- 페이지: A4 크기, 마진 0

**배포 URL**:
- 🌐 **최신 배포**: https://93f8b88f.exit-company-system-5je.pages.dev
- 🔗 **메인 사이트**: https://exit-company-system.pages.dev
- 📦 **GitHub**: https://github.com/wodnr990921-cloud/exit_company_system

**빌드 정보**:
- 빌드 크기: 150.72 kB (변경 없음)
- 빌드 시간: 2분 13초
- 배포 시간: 147.3 초
- 커밋: `feat: Update response print template with new letterhead`
